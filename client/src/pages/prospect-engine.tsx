...
function ProspectCard({ prospect, products, onStatusChange }: { prospect: Prospect; products: Product[]; onStatusChange: (id: number, status: string) => void }) {
  ...
  return (
    <Card className={`border-border/50 transition-all ${prospect.score >= 80 ? "border-l-2 border-l-rose-500" : prospect.score >= 60 ? "border-l-2 border-l-amber-500" : ""}`} data-testid={`card-prospect-detail-${prospect.id}`}>
      <div className="p-4">
        ...
        {expanded && (
          <div className="mt-3 pt-3 border-t border-border/50 space-y-3">
                ...
                {["new", "contacted", "engaged", "qualified", "closed"].map((status) => (
                  <button
                    ...
                    className={`text-[10px] px-2 py-0.5 rounded-md border transition-colors ${
                      prospect.status === status ? "bg-primary/15 text-primary border-primary/30" : "border-border/50 text-muted-foreground hover:text-foreground"
                    }`}
                  >
                    {status}
                  </button>
            ...
            <p className="text-[10px] text-muted-foreground">Last updated: {new Date(prospect.lastUpdated).toLocaleString()}</p>
          </div>
        )}
      </div>
    </Card>
...
export default function ProspectEngine() {
  ...
  const scanProspects = useMutation({
    ...
    onSuccess: (data: Prospect[]) => {
      setProspects((prev) => {
        const newIds = new Set(data.map((p) => p.companyName));
        const existing = prev.filter((p) => !newIds.has(p.companyName));
        return [...data, ...existing].sort((a, b) => b.score - a.score);
      });
      ...
      <Card className="border-border/50">
        <div className="p-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Button onClick={() => scanProspects.mutate()} disabled={scanProspects.isPending} className="sm:w-auto w-full" data-testid="button-scan-prospects">
      ...
      {prospects.length > 0 ? (
        <div className="space-y-2">
          {prospects.map((prospect) => (
            <ProspectCard key={prospect.id} prospect={prospect} products={products} onStatusChange={handleStatusChange} />
          ))}
        </div>
      ) : (
        <Card className="border-border/50">
