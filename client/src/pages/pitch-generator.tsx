  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-xl font-bold">Pitch Generator</h1><p className="text-sm text-muted-foreground">AI-powered pitch creation for the Antimatter ecosystem</p></div>
      </div>

      <Tabs defaultValue="generate" className="space-y-4">
        <TabsList>
          <TabsTrigger value="generate"><Sparkles className="w-3.5 h-3.5 mr-1.5" />Generate</TabsTrigger>
          <TabsTrigger value="history"><History className="w-3.5 h-3.5 mr-1.5" />History ({pitchHistory.length})</TabsTrigger>
        </TabsList>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <Card className="border-border/50 lg:col-span-2">
              <CardHeader className="pb-3 flex flex-row items-center justify-between">
                ...
                {pitchHistory.length > 0 && <Button variant="ghost" size="sm" onClick={() => copyToClipboard(pitchHistory[0]?.content || "")}><Copy className="w-3.5 h-3.5 mr-1.5" />Copy</Button>}
              </CardHeader>
              <CardContent>
                {generatePitch.isPending ? (
                  ...
                  <div className="space-y-3">
                    <div className="flex gap-2 flex-wrap">
                      <Badge variant="secondary">{products.find(pr => pr.id === pitchHistory[0]?.productId)?.name}</Badge>
        ...
        <TabsContent value="history" className="space-y-3">
          {pitchHistory.length === 0 ? (
            <Card className="border-border/50"><CardContent className="flex flex-col items-center justify-center py-16 text-muted-foreground"><History className="w-8 h-8 mb-3 opacity-40" /><p className="text-sm">No pitches generated yet</p></CardContent></Card>
          ) : pitchHistory.map((pitch) => (
            <Card key={pitch.id} className="border-border/50">
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex gap-2 flex-wrap">
                    <Badge variant="secondary">{products.find(pr => pr.id === pitch.productId)?.name || "Unknown"}</Badge>
                  ...
                  <Button variant="ghost" size="sm" onClick={() => copyToClipboard(pitch.content)}><Copy className="w-3.5 h-3.5" /></Button>
                </div>
