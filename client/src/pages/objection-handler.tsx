...
export default function ObjectionHandler() {
  ...
  const [objectionHistory, setObjectionHistory] = useState<Objection[]>([]);
  ...
  const handleObjection = useMutation({
    ...
    onSuccess: (data: Objection) => {
      setObjectionHistory((prev) => [data, ...prev]);
            ...
            <Card className="border-border/50 lg:col-span-1">
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea placeholder="Type the objection you're hearing from the prospect..." value={objectionText} onChange={(e) => setObjectionText(e.target.value)} className="min-h-[100px] text-sm" data-testid="input-objection" />
                </div>
                ...
                <div>
                  ...
                  <Textarea placeholder="E.g., They're a hospital with 200 beds, currently using Epic..." value={context} onChange={(e) => setContext(e.target.value)} className="min-h-[60px] text-sm" data-testid="input-obj-context" />
                </div>

                <Button className="w-full" onClick={() => handleObjection.mutate()} disabled={!selectedProduct || !objectionText || handleObjection.isPending} data-testid="button-handle-objection">
            ...
            <Card className="border-border/50 lg:col-span-2">
              <CardHeader className="pb-3 flex flex-row items-center justify-between">
                ...
                {objectionHistory.length > 0 && (
                  <Button variant="ghost" size="sm" onClick={() => copyToClipboard(objectionHistory[0]?.response || "")} data-testid="button-copy-response">
                ...
                {handleObjection.isPending ? (
                  ...
                  <div className="space-y-3">
                    <div className="flex gap-2 flex-wrap items-center">
                      ...
                      {objectionHistory[0]?.category && <Badge className={categoryColors[objectionHistory[0].category] || "bg-muted"}>{objectionHistory[0].category}</Badge>}
                    </div>
